...
export async function POST(req: Request) {
    try {
        ...
        const scored = allCoaches
            .map((coach) => {
                ...
                if (
                    ...
                    (lowered.includes("anxiety") || lowered.includes("oro"))
                )
                    score += 2;
                if (tags.includes("relation") && lowered.includes("relation")) score += 2;
                if (
                    tags.includes("grief") &&
                    (lowered.includes("grief") || lowered.includes("sorg"))
                )
                    score += 2;
        ...
        // 5) Minimal community suggestions (static, UX-friendly)
        const communities = [
            {
                id: "stress-community",
        ...
        const coachesPayload = scored.map(({ coach }) => ({
            id: coach.userId, // used by intake to route to /chat/:id, /book/:id, /coaches/:id
