...
async function CoachDashboardPage() {
    ...
    const coachId = session.user.id; // Or should we map to CoachProfile id? The requirements specify "coach".
    // Wait, the CoachProfile is linked to user.id. Let's see how bookings are saved.
    // In schema.prisma, Booking has coachId which links to CoachProfile.id or userId?
    ...
    const actualCoachId = coachProfile ? coachProfile.id : coachId; // fallback for resilience
    // Fetch Messages Context
    // We get conversations where user is a participant. 
    // And ideally only the latest message for the list.
    ...
    const formattedConversations = conversations.map((c)=>{
        ...
        // Since we don't have explicit unreadCount in schema right now, we can derive unread
        // if the last message was NOT sent by the coach, and maybe a certain time ago, but for now we will simulate it.
    ...
    // Booking table has coachId, startTime, endTime, status, type
    // In schema, coachId is String and links to CoachProfile.id
    // But if we use actualCoachId, let's just make sure.
    ...
    const formattedBookings = bookings.map((b)=>({
            ...
            status: b.status // PENDING, CONFIRMED, CANCELLED/DECLINED
        }));
    // Stats
...
//# sourceMappingURL=%5Broot-of-the-server%5D__285ffc27._.js.map